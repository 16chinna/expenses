            ...
            <div class="row">
                <div class="col-lg-12">
                    <section class="panel">
					 ...
					 {
						 ?>
						 <span class="username" style="color:red;font-weight:bold">Your Remaining Balance For Speding is = <?=$r[0]?>/-<span></br>
						 <?php
					 }
					 ?>
                         <?php
					 $re=select("select sum(limitend) from income where userid='".$_SESSION['userid']."'");
					 while($rt=mysqli_fetch_array($re))
					 {
						 ?>
						 <span class="username" style="color:red;font-weight:bold">Your Limit is = <?=$rt[0]?>/-<span>
						 <?php
					 }
					 ?>  
	...
	$n=iud($query);
	 if($n==1)
	 {
		 ...
		 if($p>=1)
		 {
			 ...
			 $y=iud("update income set limitend=limitend-'$expenseamount' where userid='".$_SESSION['userid']."'");
		 if($y>=1)
		 {
